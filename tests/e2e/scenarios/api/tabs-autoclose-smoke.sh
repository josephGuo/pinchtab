...
AUTOCLOSE_SERVER="${E2E_AUTOCLOSE_SERVER:-$E2E_SERVER}"

autoclose_lifecycle_configured() {
  e2e_curl -sf "${E2E_SERVER}/api/config" 2>/dev/null |
    jq -e '(.config // .).instanceDefaults.tabPolicy.lifecycle == "close_idle" and ((.config // .).instanceDefaults.tabPolicy.closeDelaySec | tonumber) == 1' >/dev/null 2>&1
}

if ! with_server "$AUTOCLOSE_SERVER" autoclose_lifecycle_configured; then
  echo -e "  ${YELLOW}⚠${NC} auto-close lifecycle server not configured at ${AUTOCLOSE_SERVER}, skipping"
...
with_server "$AUTOCLOSE_SERVER" autoclose_tests
