...
> [!WARNING]
> PinchTab 的仪表板、HTTP API、远程 命令行界面 目标、MCP 集成和自动化路由都属于同一个特权控制平面。它们仅适用于受信任的操作员和受信任的系统。不要将它们暴露给不受信任的用户、不受信任的客户端系统或公共互联网。
>
...
命令行界面 命令默认使用配置的本地服务器设置，`PINCHTAB_TOKEN` 可以覆盖单个 shell 会话的令牌。
